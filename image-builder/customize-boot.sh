...
echo "    past that point on every real boot. This is why the TV's pillarbox"
echo "    margins for non-16:9 content would show boot text instead of"
echo "    black. Root cause fix: stop the kernel from ever drawing to the"
echo "    framebuffer at all, rather than trying to win a timing race"
echo "    re-zeroing it afterwards -- zero-fb0 is kept as defense in depth,"
echo "    but shouldn't be relied on alone."
