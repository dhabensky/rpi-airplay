...
- **`test_video_reconnect.py`** — no commit on `dhabensky-clean-2` can
  ...
  test's own mechanism) is introduced by a later commit (`01cc470`) than
  the `skip_video_rebuild` fix it would otherwise verify (`aaad0bb`), and
  ...
  at `01cc470` (the earliest commit where `-replay` exists at all),
